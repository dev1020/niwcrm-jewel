	...
	<div class="customers-index">
		<div class="row">
			...
			<div class="col-lg-6 col-xs-12" style="background:#fff;padding:15px">
				...
				<?php Pjax::begin(['options'=>['id'=>'smsremain','data-pjax-container'=>'']]);?>
				<h4 class="alert alert-info">Remaining SMS - <strong><?= $company_sms ?></strong></h4>
				<?php Pjax::end();	?>
				...
				<div class="col-lg-12 previous" style="margin-top:10px">
						...
						<?= Select2::widget([
						'name' => 'smsprvious',
					'data' => ArrayHelper::map($promosmsTemplates,'id','sms_title'),
					'language' => 'en',
...
$(function(){
	...
	$(document).on('click','#confirmsend',function(){
		console.log(ids);
		$('.modal-body').html('<div class="alert alert-info text-center"><i style="font-size:25px" class="fa fa-cog fa-spin"></i></div>'); 
		$.post( '$send_sms_url', { ids:ids, smstext: $('#smssend').val() })
		  .done(function( data ) {
			  if(data.status){
				$('.modal').modal();
				$('.modal-body').html('<div class="alert alert-success">'+data.msg+'</div>'); 
				$('.modal-footer').html('<button type="button" class="btn btn-default pull-right" data-dismiss="modal"><i class="fa fa-times"></i> Close</button>');
				$.pjax({url: '$url', container: '#smsremain'})
			  }
		  })
		  .fail(function() {
			alert( "error" );
