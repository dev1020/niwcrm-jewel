...
<div class="customers-form" style="background:#ffffff;padding:5px">
		...
		<?php if (!Yii::$app->request->isAjax){ 
				if($billgenerated){
					echo Html::a('<i class="fa fa-trash"></i> Delete Sale', ['/station/close-session','id'=>isset($customer->id)?$customer->id:'','cust_session'=>$cust_session,'seat_id'=>isset($seat_id)?$seat_id:'','mode'=>'delete'], ['class' => 'btn btn-danger pull-right','style'=>'margin-left:20px','role'=>'modal-remote','title'=>'Delete', 
					...
					echo Html::a('<i class="fa fa-times"></i> Close Sale', ['/station/close-session','id'=>isset($customer->id)?$customer->id:'','cust_session'=>$cust_session,'seat_id'=>isset($seat_id)?$seat_id:'','mode'=>'close'], ['class' => 'btn btn-danger pull-right','style'=>'margin-left:20px','role'=>'modal-remote','title'=>'Close', 
                          'data-confirm'=>false, 'data-method'=>false,// for overide yii data api
                          'data-request-method'=>'post',
                          'data-toggle'=>'tooltip',
                          'data-confirm-title'=>'Are you sure?',
				...
				<tbody>
					...
					<tr>
						...
						<td class="text-center">
							...
							<?= Html::a('<i class="fa fa-times"></i>',['station/delete-services','id'=>$services->id,'cust_id'=>isset($customer->id)?$customer->id:'','cust_session'=>$cust_session,'seat_id'=>isset($seat_id)?$seat_id:''],['class'=>'btn btn-danger','data-id'=>$services->id,'role'=>'modal-remote','title'=>'Delete', 
                          'data-confirm'=>false, 'data-method'=>false,// for overide yii data api
                          'data-request-method'=>'post',
                          'data-toggle'=>'tooltip',
                          'data-confirm-title'=>'Are you sure?',
	...
	<div class="row">
		<div class="col-lg-12">
			<?php 
			if($billgenerated){
				echo '<div class="form-group text-center">'.\yii\helpers\Html::a('<i class="fa fa-file-text"></i> BILL', ['station/generate-bill','id'=>isset($customer->id)?$customer->id:'','session_no'=>$cust_session,'seat_id'=>isset($seat_id)?$seat_id:''], ['class' => 'btn bg-purple ','style'=>'margin-right:10px']).\yii\helpers\Html::a('Asign Executive', ['/station/assign-executive-to-customer','id'=>isset($customer->id)?$customer->id:'','cust_session'=>$cust_session,'seat_id'=>isset($seat_id)?$seat_id:''], ['class' => 'btn btn-info','style'=>'margin-right:10px','role'=>'modal-remote',]).\yii\helpers\Html::a('Add Services', ['/station/add-services-to-customer','id'=>isset($customer->id)?$customer->id:'','cust_session'=>$cust_session,'seat_id'=>isset($seat_id)?$seat_id:''], ['class' => 'btn btn-success','data-pjax'=>0,]).'</div>';
			}else{
